...
function add($name,$contactNo,$userName,$password)
{
    $con=getConnection();
    $sql="INSERT INTO employer_info (name, contactNo, userName, password) 
            VALUES ('$name', $contactNo, '$userName', '$password')";
    $result=mysqli_query($con,$sql);
    if($result)return true;
    else return false;
}
function update($name,$contactNo,$userName,$password)
{
    $con = getConnection();
